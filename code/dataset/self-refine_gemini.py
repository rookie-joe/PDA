...
def main(args):
    ...
    try:
        for i, sample in enumerate(tqdm(samples_to_process, desc="Processing samples", initial=start_index)):
            # fill input into prompt template
            prompt_input = prompt_template.replace("{gemini_output}", sample['gemini_output']).replace("{Theorem}", sample['formal'])
            ...
            if response is not None: # Check if the API call was successful
                ...
                gemini_refined_output = response.text

                # Extract sections from Gemini Pro output
                sections = extract_sections(gemini_refined_output)
                ...
                new_samples.append({
                    ...
                    "gemini_refined_output": gemini_refined_output,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prompt_template_path', type=str, default='../../prompt/fewshot_self-refine_informalization.txt', help='Path to the prompt template file')
